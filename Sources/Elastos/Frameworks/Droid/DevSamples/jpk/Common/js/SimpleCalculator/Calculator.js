...
class BigDecimal {
    constructor(n){
        return Core_New("Elastos.Math.BigDecimal", n);
    }

    static get _helper(){
        if(!BigDecimal.__helper){
            BigDecimal.__helper = Core_New("Elastos.Math.BigDecimalHelper");
        }
        return BigDecimal.__helper;
    }

    static get ZERO(){
        return BigDecimal.__helper.GetZERO();
    }
    static get ONE(){
        return BigDecimal.__helper.GetONE();
    }
    static get TEN(){
        return BigDecimal.__helper.GetTEN();
    }
    static ValueOf(n){
        return BigDecimal.__helper.ValueOf(n);
    }
}
        ...
        switch (this._currentOperation) {
            ...
            case Calculator.Operations.POW:
                if (this._leftArgument.CompareTo(this.round(
                        BigDecimal.ValueOf(Math.Pow(this._maxNumber.doubleValue(), 0.5)))) > 0 &&
                        rightArgument.CompareTo(BigDecimal.ValueOf(2)) > 0)
                    throw new TooLongValueException();
                else {
                    if (IsInteger(rightArgument))
                        ...
                        result = BigDecimal.ValueOf(Math.Pow(this._leftArgument.doubleValue(),
                                rightArgument.doubleValue()));
                    }
                }
                break;
...
//     public void onClick(View v) {
    OnClick(v) {
        ...
        elog("====OnClick====begin===="+v.GetText().ToString());
...
//                     break;
                case R.id.buttonSqrt:
                {
                    ...
                    if (this._leftArgument.CompareTo(this.BigDecimal__ZERO) < 0)
                        throw new SquareRootOfNegativeNumber();
                    this._leftArgument = BigDecimal.ValueOf(Math.Sqrt(this._leftArgument.DoubleValue()));
...
//                     break;
                case R.id.buttonDelete:
                {
                    if (this._numberTextView.getText().length > 1) {
                        ...
                        this._numberTextView.setText(this._numberTextView.getText().toString()
                                .substring(0, this._numberTextView.length - 1));
                        this._leftArgument = this.GetValue();
                    } else if (this._numberTextView.getText().length == 1)
                        this.SetZero();
                    break;
                }
...
//                     break;
                case R.id.buttonPlusMinus:
                {
                    if (this.GetValue().CompareTo(BigDecimal.valueOf(0)) != 0)
                        this._numberTextView.setText(this._numberTextView.getText().charAt(0) == '-' ?
                                this._numberTextView.getText()
                                        .subSequence(1, this._numberTextView.getText().length) :
                                "-" + this._numberTextView.getText());
                    break;
                }
...
//     }
        ...
        elog("====OnClick====end===="+v.GetText().ToString());
    }
