...
ECode EthernetNetworkFactory::DhcpRunnable::MyNetworkAgent::Unwanted()
{
    ...
    if (this == mHost->mNetworkAgent) {
        ...
        mHost->mNetworkInfo->SetDetailedState(
            NetworkInfoState_DISCONNECTED, String(nullptr), mHost->mHwAddr);
        mHost->UpdateAgent();
        mHost->mNetworkAgent = nullptr;
...
ECode EthernetNetworkFactory::DhcpRunnable::Run()
{
    ...
    AutoLock lock(mHost);
    if (mHost->mNetworkAgent != nullptr) {
    ...
    mHost->mNetworkInfo->SetDetailedState(
        NetworkInfoDetailedState_CONNECTED, String(nullptr), mHost->mHwAddr);
    ...
    /* [in] */ Boolean up)
{
    ...
    if (!up) {
        ...
        mNetworkInfo->SetDetailedState(NetworkInfoState_DISCONNECTED, String(nullptr), mHwAddr);
    }
    UpdateAgent();
    // set our score lower than any network could go
    ...
    /* [in] */ const String& iface)
{
    ...
    // try {
        ...
        if (config == nullptr) {
            Logger::E(TAG, "nullptr iterface config for %s. Bailing out.", iface.string());
            return;
        }
    ...
    /* [in] */ IStaticIpConfiguration* staticConfig)
{
    ...
    Int32 size;
    if (ipAddress != nullptr && gateway != nullptr &&
